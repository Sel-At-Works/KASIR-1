...
if(!empty($_SESSION['admin'])){
    ...
    $cek = $config->prepare("SELECT * FROM kategori WHERE nama_kategori = ? AND id_kategori != ?");
    $cek->execute([$kategori, $id]);

    if ($cek->rowCount() > 0) {
        echo '<script>window.location="../../index.php?page=kategori&error=duplikat"</script>';
    } else {
        $sql = $config->prepare("UPDATE kategori SET nama_kategori = ? WHERE id_kategori = ?");
        $sql->execute([$kategori, $id]);
    ...
    // Cek duplikat nama barang selain dirinya sendiri
    $cek = $config->prepare("SELECT * FROM barang WHERE nama_barang = ? AND id_barang != ?");
    $cek->execute([$nama, $id]);
    
    if ($cek->rowCount() > 0) {
        echo '<script>alert("Nama barang sudah ada!"); window.location="../../index.php?page=barang/edit&barang='.$id.'&error=duplikat";</script>';
        exit;
    }
